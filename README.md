...
<h3>
2. Install Python libraries (Button)
</h3>
...
## Step 3: Write a Python Script (Button)
...
- Specify the GPIO pins you would like to use for the TRIG and ECHO pins
- GPIO.setmode tells python to use GPIO numbers instead of physical pin numbers 
- GPIO.setup sets TRIG as the Output pin and ECHO as the Input pin
...
- This is the main loop where it will allow the sensor to detect the distances through the use of sound waves travelling back and forth
<br></br>
- The TRIG pin starts the measurement making the sensor send a ultrasonic "ping"
<br></br>
- The ECHO pin will then go HIGH next, recording the start time the sound wave/ping left the sensor 
<br></br>
- After the sound wave has returned to the sensor, the ECHO pin will go LOW and record the stop time of when the wave returns
<br></br>
- The start and stop time will then run the "Calculating distance" formula and return the distance in Centimeters.



## Step 5: Configuration between Raspi and Phidgets RFID readers
